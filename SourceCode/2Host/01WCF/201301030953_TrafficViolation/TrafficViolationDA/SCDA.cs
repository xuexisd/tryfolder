...
namespace TrafficViolationDA
{
    ...
    public class SCDA : ISC
    {
        ...
        public List<SCModel> GetCompleted(SCParams scParams)
        {
            ...
            try
            {
                CarInfoModel isExistCarInfoModel = GetCarInfoDetail(scParams.CarNumber);
                if (isExistCarInfoModel == null)
                {
                    isExistCarInfoModel = new CarInfoModel();
                    isExistCarInfoModel.CarFrame = scParams.CarFrame = GetRandomNumber(100000, 999999).ToString();
                ...
                bool isRefresh = NeedRefresh(scParams.CarNumber,"C");
                if (isRefresh)
                {
                ...
                List<ViolationModel> dbListViolationModel = GetViolation(scParams.CarNumber, "C");
        ...
        public List<SCModel> GetUnProcessed(SCParams scParams)
        {
            ...
            try
            {
                ...
                bool isRefresh = NeedRefresh(scParams.CarNumber,"U");
                if (isRefresh)
                {
                ...
                List<ViolationModel> dbListViolationModel = GetViolation(scParams.CarNumber, "U");
                foreach (ViolationModel currentModel in dbListViolationModel)
                {
        ...
        private CarInfoModel GetCarInfoDetail(string carNumber)
        {
            SqlHelper helper = new SqlHelper();
            var sqlReader = helper.ExecuteReader("P_CarInfo_GetDetail_By_CarNum", carNumber, "");
        ...
        private bool NeedRefresh(string carNumber, string ViolationStatus)
        {
            bool retIsRefresh = false;
            SqlHelper helper = new SqlHelper();
            if (ViolationStatus == "C")
            {
                var sqlReader = helper.ExecuteReader("P_Refresh_Date_C", carNumber);
            ...
            {
                var sqlReader = helper.ExecuteReader("P_Refresh_Date_U", carNumber);
                DateTime dbDate = DateTime.Now;
                while (sqlReader.Read())
                {
                    dbDate = DateTime.Parse(sqlReader[0].ToString());
        ...
        private List<ViolationModel> GetViolation(string carNumber, string ViolationStatus)
        {
            List<ViolationModel> listModel = new List<ViolationModel>();
            SqlHelper helper = new SqlHelper();
            DataSet ds = new DataSet();
            ds.Tables.Add("Violation");
            helper.FillDataset("P_TrafficViolation_Get_By_CarNum", ds.Tables["Violation"], carNumber, "123456", ViolationStatus);
