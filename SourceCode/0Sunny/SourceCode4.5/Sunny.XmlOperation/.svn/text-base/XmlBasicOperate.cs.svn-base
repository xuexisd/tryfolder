...
namespace Sunny.XmlOperation
{
    public class XmlBasicOperate : IDisposable
    {
        //以下为单一功能的静态类
        ...
        /// <param name="Xmlitemorindex">读取节点的索引</param>
        /// <returns></returns>
        public static string ReadXmlReturnNode(string XmlPath,string Node,int Xmlitemorindex)
        {
            XmlDocument docXml = new XmlDocument();
            docXml.Load(@XmlPath);
            XmlNodeList xn = docXml.GetElementsByTagName(Node);
            return xn.Item(Xmlitemorindex).InnerText.ToString();           
        }
        #endregion
